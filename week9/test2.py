# investigate temporal binning for noise reduction
...
# get differential light curve
result = differential_photometry(target_temperature, 4000, "lsst2023-g", n_exposures, exposure_time, starting_photons)
...
for b in bin_sizes:
    ...
    binned = bin_data(result['diff_lc'], b)

    binned_stds.append(np.std(binned))

    theoretical.append(np.std(result['diff_lc']) / np.sqrt(b))

print("temporal binning")
print(f"{'Bin Size':>10} {'Std Dev':>12} {'Theoretical':>12}")

for b, std, theo in zip(bin_sizes, binned_stds, theoretical):
    print(f"{b:10d} {std:12.6f} {theo:12.6f}")

# plot
plt.figure(figsize=(8,6))

plt.plot(bin_sizes, binned_stds, 'o-', linewidth=2, markersize=8, label="Measured")

plt.plot(bin_sizes, theoretical, '--', linewidth=2, alpha=0.7, label="Theoretical 1/√N")
